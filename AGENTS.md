...
| 경로 | 역할 |
|---|---|
| `.Codex/commands/` | 슬래시 커맨드 (`/build`, `/test`, `/check`, `/pr`, `/new-feature`) |
| `.Codex/skills/` | 자동 트리거되는 Skill (`new-feature-scaffold`) |
| `.Codex/agents/` | Stop 훅 등에서 호출되는 서브에이전트 정의 (`troubleshooting-recorder`) |
| `.Codex/TROUBLESHOOTING.md` | 트러블슈팅·트레이드오프·성능 개선 로그 (Stop 훅이 매 턴 자동 갱신) |
| `.Codex/settings.json` | 팀 공유 Codex 설정 (체크인) — 훅 등록 등 |
| `.Codex/settings.local.json` | 개인 권한 설정 (gitignore — 체크인 금지) |

## Troubleshooting Log

`.Codex/TROUBLESHOOTING.md` 는 **트러블슈팅·트레이드오프·성능 개선** 기록의 단일 출처. PR 본문/커밋 메시지에 묻혀 사라지는 "왜 이 코드인가" 와 "재발 시 어떻게 풀었나" 를 한 곳에 모은다.

- **자동 기록**: `.Codex/settings.json` 의 Stop 훅이 매 턴 종료 시 `troubleshooting-recorder` 에이전트(`.Codex/agents/troubleshooting-recorder.md`) 를 띄움. 에이전트가 세션 transcript 를 읽어 조건 충족 시 파일 상단에 항목 prepend. 사용자는 대기하지 않음.
...
- **`main`에 직접 커밋/푸시 금지.** 항상 브랜치 → PR → 머지.
- 기능 구현 시 반드시 해당 기능의 **테스트 코드**도 함께 작성한다.
  - 모든 테스트는 `core:testing` 모듈을 활용한다.
- 기능 작업 완료 시 관련 **문서(AGENTS.md)도 함께 업데이트**한다:
  - 새 모듈 추가 → 루트 `AGENTS.md` Module Structure 업데이트
  - 새 feature 추가 → `feature/AGENTS.md` 또는 루트 Module Structure 반영
  - 새 core 모듈/API 추가 → `core/AGENTS.md` 의존성 규칙 및 사용법 업데이트
  - Convention plugin 변경 → 루트 `AGENTS.md` Convention Plugins 테이블 업데이트
...
| `docs/` | 문서만 변경 | `docs/update-Codex-md` |
...
- 커밋 메시지, PR 본문, 코드 주석 등 **git 기록 어디에도 Codex/AI 관련 언급 금지**
  - `Co-Authored-By: Codex ...`
  - `🤖 Generated with Codex`
  - "AI assistant", "with help of Codex" 류
- `--no-verify` (pre-commit hook 우회) 금지 (사용자 명시 요청 시 예외)
- `main` 브랜치 직접 push, force push 금지
